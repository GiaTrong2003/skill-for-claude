...
| **WAL / Redo log** | Write-Ahead Log — ghi log trước khi commit data, giúp khôi phục khi crash. MariaDB InnoDB gọi là **redo log** (file `ib_logfile*`), thêm **binlog** (binary log) để replicate sang slave. |
| **SHOW PROCESSLIST** | Lệnh MariaDB liệt kê mọi connection đang làm gì — tương đương `pg_stat_activity` của Postgres. Chi tiết hơn: `information_schema.PROCESSLIST`. |
| **performance_schema** | Schema nội bộ MariaDB chứa metric runtime: query thống kê, lock, IO — dùng để chẩn đoán performance. |
| **KILL** | Lệnh MariaDB giết session/query theo thread id. `KILL QUERY <id>` chỉ ngắt query hiện tại (giữ connection); `KILL <id>` giết hẳn connection. |
| **max_statement_time** | Setting MariaDB giới hạn thời gian tối đa 1 query chạy (giây, float). Tương đương `statement_timeout` của Postgres. Tránh query treo vô hạn. |
...
**Kéo về thực tế**: Trong hệ thống Hợp đồng điện tử em làm, em dùng **MariaDB** (InnoDB engine) cho metadata hợp đồng & chữ ký (cần ACID — 1 hợp đồng có 3–5 bên ký, service crash giữa chừng phải rollback sạch), **MongoDB** cho audit log (schema biến thiên theo loại event, ghi nhiều đọc ít, giữ 10 năm), **Elasticsearch** (bộ search engine chuyên xử lý full-text) cho tìm nội dung hợp đồng. Không có lựa chọn "SQL vs NoSQL" tổng thể — mỗi loại data quyết riêng.

**Câu xoáy đáp xoay**:
- *"MariaDB có kiểu JSON rồi, sao không dùng luôn cho audit log, tránh học Mongo?"* → Đúng, MariaDB `JSON` type (thực chất là LONGTEXT + validation) là lựa chọn tốt nếu team đã thạo MariaDB. Em vẫn chọn Mongo vì lúc đó team có sẵn kinh nghiệm, và volume ghi audit rất lớn — Mongo ghi nhanh hơn JSON column của MariaDB ở scale này (MariaDB JSON không phải binary format như Postgres JSONB, query/filter chậm hơn). Nếu làm lại, em sẽ PoC cả 2 để đo trước khi quyết.
- *"Em nói ACID mạnh — isolation level em đặt là gì?"* → Mặc định MariaDB InnoDB là **`REPEATABLE READ`** (trong 1 transaction luôn thấy snapshot nhất quán — khác Postgres default `READ COMMITTED`). Luồng ký của em giữ default này để tránh **phantom read** (đọc lại cùng query ra kết quả khác khi có thêm row). Với workload đọc nhiều (report), em set session thành `READ COMMITTED` để giảm lock contention.
- *"Mongo từ 4.0 đã có multi-document transaction, em biết không?"* → Biết, nhưng có ràng buộc — chỉ trong 1 replica set hoặc sharded cluster cùng shard, và overhead cao. Em chưa dùng vì nếu cần transaction thì đã dùng MariaDB.
- *"Nếu 1 ngày phải sharding MariaDB em làm sao?"* → 2 cách: (1) **Spider storage engine** (MariaDB có sẵn — shard theo range/hash, query được route tự động tới shard đúng, tương tự Citus của Postgres), (2) sharding tay ở application layer với shard key hợp lý (em sẽ chọn `tenant_id`) — phổ biến hơn Spider vì control tốt hơn. Multi-master active-active có thể dùng **Galera Cluster** (synchronous replication), nhưng write latency cao hơn. Rủi ro lớn nhất là **cross-shard transaction** — cần dùng saga (chuỗi giao dịch nhỏ bù trừ) thay vì 2PC (two-phase commit, chậm và dễ block).
...
3. **Khi index LÀM CHẬM**:
   - **Write amplification** (khuếch đại ghi): mỗi INSERT/UPDATE phải cập nhật tất cả index → tăng latency, tăng **redo log + binlog** (với MariaDB InnoDB — log ghi trước khi commit, log replicate sang slave).
   ...
   - **Index bloat** (phình): sau nhiều DELETE/UPDATE, cây phình lên dù ít data thật → với MariaDB InnoDB chạy `OPTIMIZE TABLE <table>` (thực chất là `ALTER TABLE ... ENGINE=InnoDB`) để rebuild, hoặc `ALTER TABLE ... FORCE` để reorganize.

**Kéo về thực tế**: Bảng `signature` của dự án ~200M hàng, partition (chia bảng) theo tháng. API "list hợp đồng của tenant theo trạng thái" **p95** (percentile 95 — 95% request nhanh hơn, 5% chậm hơn) nhảy từ 200ms lên 2s. Em chạy `EXPLAIN FORMAT=JSON` + `ANALYZE` (MariaDB 10.1+ có `ANALYZE SELECT ...` in ra kế hoạch thực thi thật + thời gian mỗi bước) → **full table scan** (quét tuần tự toàn bảng) do index cũ `(tenant_id)` selectivity thấp (1 tenant chiếm 30% data). Em đổi thành composite `(tenant_id, status, created_at DESC)` cover luôn sort, đồng thời DROP index `(status)` đơn lẻ — đỡ write 15%. p95 về 50ms.
...
- *"Em tạo index trên bảng 200M hàng — không sợ lock bảng lâu à?"* → Dùng `ALTER TABLE ... ADD INDEX ... ALGORITHM=INPLACE, LOCK=NONE` (hoặc `CREATE INDEX ... ALGORITHM=INPLACE, LOCK=NONE`) — MariaDB InnoDB cho phép **online DDL** từ version 5.6+, không khóa bảng cho write. Chậm hơn 2–3× và tốn disk tạm. Em chạy ngoài giờ cao điểm, monitor qua `SHOW PROCESSLIST` (xem state `copy to tmp table` hay `enabling keys`). Nếu muốn detail hơn dùng `performance_schema.events_stages_current`.
- *"Làm sao em biết index nào đang không được dùng để drop?"* → MariaDB có `performance_schema.table_io_waits_summary_by_index_usage` (phải bật `performance_schema=ON`) — cột `count_star=0` sau 1 tháng nghĩa là không có query nào dùng. Hoặc dùng view tiện hơn `sys.schema_unused_indexes`. Tuy nhiên phải cẩn thận: index có thể dùng cho enforcement unique constraint hoặc cho query chạy hàng tháng (report cuối tháng).
- *"Nếu đang peak mà không thể DROP index thì em handle sao?"* → (1) **Invisible index** (MariaDB 10.6+): `ALTER TABLE t ALTER INDEX idx_name INVISIBLE` — optimizer không dùng, nhưng index vẫn được maintain; test impact vài ngày, nếu OK mới DROP thật. (2) `DROP INDEX ... ALGORITHM=INPLACE, LOCK=NONE` — không lock bảng. (3) Hoặc chờ giờ thấp điểm.
- *"Partial index là gì? MariaDB có hỗ trợ không?"* → **Partial index** chỉ đánh index tập con thỏa điều kiện — ví dụ Postgres: `CREATE INDEX ... WHERE status='ACTIVE'`. **MariaDB không có native partial index** (tính đến 10.11). Workaround: (1) **Prefix index** với cột `VARCHAR` (index chỉ N ký tự đầu), (2) tạo **generated column** `is_active_status` rồi index cột đó, (3) với MariaDB 10.5+ dùng **application period** hoặc partition để loại bỏ subset. Không elegant bằng Postgres nhưng làm được.

> **Clarify**: "Anh muốn em nói về MariaDB, MySQL, hay nguyên lý chung ạ? (Hầu hết MariaDB và MySQL giống nhau ở index, nhưng MariaDB có thêm một số feature riêng như invisible index 10.6+)"
> **Nếu chưa chắc** (vd: Oracle/Postgres): "Em chủ yếu hands-on MariaDB, nhưng B-tree là chung. Postgres có thêm `CREATE INDEX CONCURRENTLY` và partial index native — em hiểu khái niệm nhưng chưa tune ở production."
...
2. **Thực tế mạng luôn có thể phân vùng** (đứt cáp, switch chết) → **P là bắt buộc**. Câu hỏi thật: chọn CP hay AP khi partition xảy ra?
   - **CP**: từ chối request để không trả data sai — Zookeeper, etcd, HBase, MariaDB Galera Cluster (synchronous replication), hoặc MariaDB semi-sync replication với `rpl_semi_sync_master_wait_point=AFTER_SYNC`.
...
- **Hợp đồng & chữ ký → CP**. Tuyệt đối không để 2 node thấy 2 trạng thái khác nhau (A thấy "đã ký", B thấy "chưa ký" → hợp đồng có thể bị khiếu nại pháp lý). MariaDB **semi-sync replication** (master chờ ít nhất 1 slave ack đã nhận binlog mới coi là commit — set `rpl_semi_sync_master_enabled=1`), hoặc **Galera Cluster** (sync đồng bộ thật sự — write commit trên tất cả node mới return OK). Nếu replica mất kết nối → write reject hoặc degrade về async (tùy timeout config).
...
**Câu xoáy đáp xoay**:
- *"MariaDB semi-sync có phải CP thật không? Nếu replica sync mất kết nối thì sao?"* → **Không phải CP tuyệt đối** — MariaDB semi-sync có `rpl_semi_sync_master_timeout` (mặc định 10s): nếu replica không ack trong thời gian đó, master **tự fallback về async** (vẫn commit, data lệch) — đây là **AP behavior** dưới áp lực. Muốn CP đúng nghĩa cần **Galera Cluster** (3+ node, write cần quorum đa số) hoặc **MaxScale + semi-sync** với timeout lớn + monitoring chặt. Trade-off: Galera latency cao hơn (~2–5× async) vì phải round-trip mọi write.
...
3. **Top offender**: `SHOW FULL PROCESSLIST` hoặc `SELECT * FROM information_schema.PROCESSLIST ORDER BY TIME DESC` → tìm query/transaction treo lâu. Kết hợp `information_schema.INNODB_TRX` (xem transaction đang chạy + time bắt đầu).
...
**Kéo về thực tế**: Có lần dự án em DB CPU 90% lúc 9h sáng. Em chạy `SHOW FULL PROCESSLIST` + `SELECT * FROM information_schema.INNODB_TRX` thấy 1 session INSERT treo 5 phút, transaction id cụ thể. Trace sang code, thấy block `catch` thiếu rollback sau deploy hôm trước. Em `KILL <thread_id>` → CPU về 40%. Sau đó raise hotfix PR thêm rollback + alert transaction treo > 30s.

**Câu xoáy đáp xoay**:
- *"Nếu `SHOW PROCESSLIST` không cho thấy query nặng — CPU vẫn cao thì điều tra sao?"* → (1) `performance_schema.events_statements_summary_by_digest` (gộp query theo template "digest", thống kê `sum_timer_wait`, `count_star`) — tìm query nhiều người gọi (mỗi lần rẻ nhưng tổng nặng). Hoặc bật **slow query log** với `long_query_time=0.5s`. (2) `perf top` trên OS — xem CPU tốn vào process gì (có thể InnoDB purge thread hoặc background merge). (3) IOPS disk — `SHOW ENGINE INNODB STATUS` có section `BUFFER POOL AND MEMORY` + `FILE I/O`.
- *"performance_schema có overhead không?"* → Có, ~5–10% CPU nếu bật nhiều instrument. Nhưng ở production nên bật consumers cơ bản (`statements_digest`, `statements_current`) — lợi ích chẩn đoán lớn hơn chi phí. MariaDB mặc định bật performance_schema từ 10.x.
- *"Phân biệt lock wait vs query nặng thế nào?"* → `information_schema.INNODB_LOCK_WAITS` (MariaDB < 10.5) hoặc `performance_schema.data_lock_waits` (MariaDB 10.5+) — cho biết transaction nào đang chờ lock của transaction nào. Kết hợp `SHOW ENGINE INNODB STATUS` section `TRANSACTIONS` xem lock chi tiết. Nếu không thấy lock wait → query đang chạy thật (compute-bound hoặc IO-bound).
...
- *"Kill query giữa production có an toàn không?"* → MariaDB có 2 flavor: `KILL QUERY <thread_id>` (signal nhẹ — ngắt query hiện tại, giữ connection + transaction) an toàn. `KILL <thread_id>` (giết cả connection) nặng hơn — nếu đang trong transaction sẽ rollback. Nguy cơ: query đang trong transaction, kill thì rollback → data không bị sai (ACID). Nhưng nếu là `ALTER TABLE` online quan trọng, kill là phí công vì phải chạy lại từ đầu.
- *"Plan đổi đột ngột là sao?"* → Optimizer dựa vào statistics trong `mysql.innodb_table_stats` + `mysql.innodb_index_stats` để chọn plan. InnoDB tự recalc khi data thay đổi 10% (setting `innodb_stats_auto_recalc=ON`). Statistics mới có thể làm optimizer chọn plan tệ hơn (bảng bigger, cardinality đổi). Fix: (1) `ANALYZE TABLE <table>` thủ công ngoài giờ cao điểm, (2) `ALTER TABLE ... STATS_PERSISTENT=1, STATS_SAMPLE_PAGES=64` (tăng độ chi tiết sample), (3) MariaDB có **optimizer hints** native: `SELECT /*+ INDEX(t, idx_name) */ ...` hoặc `FORCE INDEX (idx_name)`.
...
**Kéo về thực tế**: Hệ thống em tách 4 service chính: `contract-service` (CRUD + workflow), `signing-service` (gọi Viettel-CA / SmartCA / USB token — khác SLA và dependency ngoài), `notification-service`, `audit-service`. Không tách nhỏ hơn vì team em ~15 người, tách nhỏ hơn là overhead thừa. Phần billing-internal em giữ trong contract-service vì transaction luôn chung 1 DB MariaDB, tách ra thì phải saga phức tạp.

**Câu xoáy đáp xoay**:
- *"Modular monolith cụ thể tổ chức code thế nào?"* → (1) Mỗi module 1 package riêng (`com.company.contract`, `com.company.signing`), (2) **Public API** rõ — chỉ được gọi qua interface public, không gọi internal class, (3) **Database riêng** mỗi module — MariaDB không có "schema" như Postgres, thay bằng **database riêng** (`CREATE DATABASE contract; CREATE DATABASE signing;`) trên cùng instance, hoặc table prefix (`contract_*`, `signing_*`). Không JOIN cross-module. (4) Enforcement bằng **ArchUnit** (Java library kiểm tra kiến trúc lúc test) hoặc lint rule.
...
- *"Cost tăng theo 100× user — em quản trị thế nào?"* → (1) Measure trước, tối ưu query/index/cache để **cost không linear với user** — ideal là sub-linear. (2) **Tier-ed storage**: hot data MariaDB SSD, warm data đẩy S3 cold (archive). Có thể dùng MariaDB **ColumnStore** cho analytics/archive thay vì InnoDB. (3) **Autoscaler với max cap** để không bill runaway. (4) **Unit economics**: `cost per user` tracking hàng tháng — nếu tăng đột biến → alert cost.
...
4. **Kiến trúc luồng**:
   ...
   - Ghi **event log** vào Kafka (topic `topup.request`, partition theo `msisdn`) NGAY — đây là "điểm không mất". Cơ chế giống WAL/binlog của DB, nhưng ở tầng message queue.
   ...
   - Ghi kết quả vào DB (MariaDB cho bản ghi nghiệp vụ) + publish `topup.completed`.
5. **Mô hình dữ liệu**:
...
5. **Mô hình dữ liệu**:
   ...
   - Billing aggregate lưu trong MariaDB / ClickHouse cho BI (Business Intelligence — báo cáo). Với khối lượng analytics lớn em sẽ đẩy sang MariaDB **ColumnStore** (OLAP engine — tối ưu cho aggregation) hoặc ClickHouse.
6. **Báo cáo theo giờ**:
